...
Each time the user completes a ranked activity the users rank progress is updated
based off of the activity's rank. The progress earned from the completed activity 
is relative to what the user's current rank is compared to the rank of the activity
A user's rank progress starts off at zero, each time the progress reaches 100 the
 user's rank is upgraded to the next level
Any remaining progress earned while in the previous rank will be applied towards
the next rank's progress (we don't throw any progress away). 
...
Completing an activity that is ranked the same as that of the user's will
 be worth 3 points
Completing an activity that is ranked one ranking lower than the user's will 
be worth 1 point
Any activities completed that are ranking 2 levels or more lower than the user's
 ranking will be ignored
...
public class User {
	...
	public void incProgress(int otherRank){
		if(otherRank< -8 || otherRank >8 || otherRank == 0)
			throw new IllegalArgumentException("Wrong input");
		int diff = otherRank - this.rank;
		if(otherRank>0 && rank<0)
			diff = diff -1;
		else if(rank == 1 && otherRank ==-1)
		    diff = -1;
		if(diff>0)
		...
		if(progress>=100){
			if(rank<8){
				...
				if(val>0 || (val == 0 && rank == 8))
					progress-= progress*(rem - val);
				else
					progress = 0;
			}
		}
